"""
Train a skin type classifier using Derm Foundation embeddings.

This is a SINGLE-LABEL MULTI-CLASS problem (4 classes: dry, normal, oily, redness).
Different from conditions which is multi-label.
...
Run: python scripts/train_skintype_mlp.py
"""
...
# MLP Architecture (smaller than top-10 due to smaller dataset)
HIDDEN_UNITS = [128, 64]  # Smaller than [256, 128] to reduce overfitting
...
MODELS_DIR = Path(__file__).parent.parent / "app" / "models" / "skintype"
...
def load_data() -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    emb_path = DATA_DIR / "skintype_embeddings.npz"
    
    if not emb_path.exists():
        raise FileNotFoundError(
            f"Embeddings not found at {emb_path}. "
            f"Run generate_skintype_embeddings.py first."
        )
...
def train_mlp_fold(X_train, y_train, X_val, y_val, scaler, label_encoder, fold_num):
    ...
    model = SkinTypeMLP(
        ...
        num_classes=len(SKIN_TYPE_CLASSES),
...
def cross_validate(X, y, model_type='both'):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        if model_type in ['mlp', 'both']:
            ...
            label_encoder.fit(SKIN_TYPE_CLASSES)
...
def train_final_model(X, y, model_type='mlp'):
    ...
    if model_type == 'mlp':
        ...
        # Wrap for inference
        wrapper = SkinTypeMLPWrapper(model, scaler, SKIN_TYPE_CLASSES)
    ...
    print(classification_report(y_test, y_pred, target_names=SKIN_TYPE_CLASSES))
    
    print("Confusion Matrix:")
    cm = confusion_matrix(y_test, y_pred, labels=SKIN_TYPE_CLASSES)
    print(f"{'':>10} " + " ".join(f"{c[:4]:>6}" for c in SKIN_TYPE_CLASSES))
    for i, cls in enumerate(SKIN_TYPE_CLASSES):
        print(f"{cls:>10} " + " ".join(f"{cm[i,j]:>6}" for j in range(len(SKIN_TYPE_CLASSES))))
...
def main():
    print("=" * 60)
    print("SKIN TYPE CLASSIFIER TRAINING")
    print("=" * 60)
    ...
    if best_model_type == 'mlp':
        joblib.dump(wrapper, MODELS_DIR / "skintype_mlp_model.joblib")
        joblib.dump(scaler, MODELS_DIR / "skintype_mlp_scaler.joblib")
    else:
        joblib.dump(wrapper, MODELS_DIR / "skintype_logreg_model.joblib")
        joblib.dump(scaler, MODELS_DIR / "skintype_logreg_scaler.joblib")
    ...
    with open(MODELS_DIR / "skintype_config.json", "w") as f:
    ...
    print(f"Improvement over random: {(test_acc - 0.25) / 0.25:.0%}")
